...
serve(async (req) => {
  ...
  try {
    ...
    if (superadminError || !isSuperadmin) {
      logStep('Superadmin check failed', { corr, error: superadminError?.message, isSuperadmin })
      return fail(403, 'FORBIDDEN', 'Superadmin access required', 'Only superadmins can reconcile billing');
    }

    logStep('Superadmin access verified', { corr, userId: authUser.id })
    ...
    // Keep a reference for downstream compatibility
