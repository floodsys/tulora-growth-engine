...
serve(async (req) => {
  ...
  try {
    ...
    // Verify user has admin access to this org or is superadmin (DB RPC is source of truth)
    const { data: isSuperadmin } = await supabase.rpc('is_superadmin', { user_id: userData.user.id })
