-- Database schema updates for owner role management
...
-- 5. Create function to check if removal would leave org without admins
...
BEGIN
  ...
  VALUES (p_org_id, p_new_owner_user_id, 'admin'::org_role, true)
  ON CONFLICT (organization_id, user_id) 
  DO UPDATE SET role = 'admin'::org_role, seat_active = true;
  
  -- Optionally keep old owner as admin
  IF p_keep_old_owner_as_admin AND old_owner_id != p_new_owner_user_id THEN
    INSERT INTO public.organization_members (organization_id, user_id, role, seat_active)
    VALUES (p_org_id, old_owner_id, 'admin'::org_role, true)
    ON CONFLICT (organization_id, user_id) 
    DO UPDATE SET role = 'admin'::org_role, seat_active = true;
  END IF;
...
-- 7. Update RLS policies to protect owner membership

-- Create policy to prevent owner membership deletion
...
-- Create policy to prevent owner role changes
...
-- Create policy to prevent last admin removal
