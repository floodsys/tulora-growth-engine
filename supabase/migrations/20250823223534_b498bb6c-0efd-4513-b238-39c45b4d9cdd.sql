...
-- Drop ALL existing policies that might depend on role columns
...
-- Normalize legacy roles (guarded)
DO $$
DECLARE t regtype;
BEGIN
  IF to_regclass('public.organization_members') IS NULL THEN RETURN; END IF;
  SELECT a.atttypid::regtype INTO t FROM pg_attribute a WHERE a.attrelid='public.organization_members'::regclass AND a.attname='role' AND a.attnum>0 AND NOT a.attisdropped;
  IF t IS NOT NULL AND t::text NOT IN ('org_role','public.org_role') THEN
    EXECUTE 'UPDATE public.organization_members SET role = ''admin'' WHERE role::text IN (''owner'',''Owner'',''OWNER'')';
    EXECUTE 'UPDATE public.organization_members SET role = lower(role::text)';
  END IF;
END $$;

-- Drop default and change role column to use enum (NO-ALTER guard)
DO $$
DECLARE t regtype;
BEGIN
  IF to_regclass('public.organization_members') IS NULL THEN RETURN; END IF;
...
-- Recreate minimal necessary policies
