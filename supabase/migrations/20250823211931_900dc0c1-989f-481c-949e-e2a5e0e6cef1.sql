...
BEGIN
    ...
    IF lower(trim(p_role)) NOT IN ('admin', 'editor', 'viewer', 'user') THEN
        RAISE EXCEPTION 'Invalid role: %. Must be one of: admin, editor, viewer, user', p_role
            USING ERRCODE = 'invalid_parameter_value';
    END IF;
    
    normalized_role := lower(trim(p_role))::public.org_role;
    
    -- Generate cryptographically strong random token (base64url encoded)
