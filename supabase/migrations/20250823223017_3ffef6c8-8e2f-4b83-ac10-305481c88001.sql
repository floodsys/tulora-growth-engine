...
-- Drop default before changing type, then change role column to use enum (NO-ALTER guard)
DO $$
DECLARE t regtype;
BEGIN
  IF to_regclass('public.organization_members') IS NULL THEN RETURN; END IF;
...
-- Change status column to use enum (guarded)
DO $$
BEGIN
  IF to_regclass('public.organization_invitations') IS NULL THEN RETURN; END IF;
...
-- Normalize any legacy data in memberships table (guarded)
DO $$
BEGIN
  IF to_regclass('public.memberships') IS NULL THEN RETURN; END IF;
  -- memberships.role is text, not enum - safe to update
