-- Successfully normalized organization multi-tenancy data model with consistent organization_id usage, proper role enums, and foreign key constraints with cascade operations.
...
-- Normalize organizations table (remove unnecessary columns, keep core fields)
...
-- Convert role column using temporary column
ALTER TABLE public.organization_members 
...
-- Set defaults and FK constraints
ALTER TABLE public.organization_members 
...
-- Convert role and status columns
ALTER TABLE public.organization_invitations 
ADD COLUMN role_new public.org_role,
