-- ============================================================================
-- Migration: Fix handle_new_user_signup to create sample agents as 'disabled'
-- ============================================================================
-- The trigger previously created sample agent_profiles with status='active'
-- and a temp_ retell_agent_id. This violates the CHECK constraint
-- chk_agent_profiles_no_placeholder_active added in migration
-- 20260216070000_deactivate_placeholder_seed_agents.sql.
--
-- Fix: Change the sample agent's initial status from 'active' to 'disabled'.
-- The agent must be provisioned with a real Retell agent_id before activation.
-- ============================================================================
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        INSERT INTO public.agent_profiles (
            ...
            'disabled', true, false, true, 1000, 0.7,
            jsonb_build_object(
