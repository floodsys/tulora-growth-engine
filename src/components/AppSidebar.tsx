    ...
    >
      <SidebarContent className="flex flex-col h-full">
        <div className="flex-1 space-y-4">
          <SidebarGroup>
            <SidebarGroupLabel className={isMobile ? "px-4 py-4 mb-2" : state === "collapsed" ? "pl-4 pr-2 py-4 mb-3" : "px-6 py-6 mb-3"}>
            ...
            <SidebarGroupContent className={state === "collapsed" && !isMobile ? "pl-4 pr-2" : "px-3"}>
              <SidebarMenu className="space-y-1">
                {items.map((item) => (
                  <SidebarMenuItem key={item.title}>
                    <SidebarMenuButton 
                      onClick={() => setActiveScreen(item.url)}
                      className={`h-10 ${state === "collapsed" && !isMobile ? "pl-4 pr-2" : "px-3"} ${activeScreen === item.url ? "bg-muted text-primary font-medium" : "hover:bg-muted"}`}
                    >
        ...
        <div className="mt-auto border-t pt-4">
          ...
          {state === "collapsed" && !isMobile && (
            <SidebarGroup>
              <SidebarGroupContent className="pl-4 pr-2">
                <SidebarMenu className="space-y-1">
                  <SidebarMenuItem>
                    <SidebarMenuButton 
                      onClick={() => setActiveScreen("notifications")}
                      className={`h-10 pl-4 pr-2 ${activeScreen === "notifications" ? "bg-muted text-primary font-medium" : "hover:bg-muted"}`}
                    >
                      <Bell className="h-4 w-4" />
                    </SidebarMenuButton>
                  </SidebarMenuItem>
                  <SidebarMenuItem>
                    <SidebarMenuButton className="h-10 pl-4 pr-2 hover:bg-muted">
                      <HelpCircle className="h-4 w-4" />
                    </SidebarMenuButton>
                  </SidebarMenuItem>
                </SidebarMenu>
          ...
          <div className={state === "collapsed" && !isMobile ? "pl-4 pr-2 py-2" : "p-3"}>
