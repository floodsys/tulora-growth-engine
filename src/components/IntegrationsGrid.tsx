      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="absolute inset-0 pointer-events-none">
          {integrationLogos.map((logo, index) => (
            <div
              key={index}
              className={`absolute ${logo.position} w-12 h-12 bg-background-secondary rounded-xl flex items-center justify-center shadow-sm border border-border animate-float opacity-60 hover:opacity-100 transition-opacity duration-300`}
              style={{
        ...
        <div className="text-center relative z-10 max-w-4xl mx-auto">
          ...
          <Button 
            size="lg" 
            className="bg-brand hover:bg-brand-dark text-white px-8 py-4 text-lg font-semibold rounded-full shadow-lg hover:shadow-xl transition-all duration-300 group"
          >
            Discover Now
